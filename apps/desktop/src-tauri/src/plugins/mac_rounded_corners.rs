...
// ─── Custom add: hide native traffic lights ─────────────────────
//
// `enable_modern_window_style` re-enables NSClosableWindowMask /
// NSMiniaturizableWindowMask / NSResizableWindowMask in the
// NSWindow style mask so the native traffic-light buttons render.
// We want the rounded corners + content-view layer mask the plugin
// provides BUT render our own DesktopTitleBar buttons in the SPA
// instead — so call this after enable_modern_window_style to
// hide the three native buttons via setHidden:YES.  Re-rendering
// (e.g. after fullscreen transitions) may un-hide them; if that
// becomes a problem we can listen for resize events SPA-side and
// reinvoke this command.
#[tauri::command]
pub fn hide_traffic_lights<R: Runtime>(
    _app: AppHandle<R>,
    window: WebviewWindow<R>,
) -> Result<(), String> {
    #[cfg(target_os = "macos")]
        ...
        window
            .with_webview(move |webview| {
                ...
                unsafe {
                    ...
                    let mini: id = msg_send![ns_window, standardWindowButton: 1];
                    let zoom: id = msg_send![ns_window, standardWindowButton: 2];
                    if !close.is_null() {
                        let _: () = msg_send![close, setHidden: cocoa::base::YES];
                    }
                    if !mini.is_null() {
                        let _: () = msg_send![mini, setHidden: cocoa::base::YES];
                    }
                    if !zoom.is_null() {
                        let _: () = msg_send![zoom, setHidden: cocoa::base::YES];
                    }
                }
            })
            .map_err(|e| e.to_string())?;
