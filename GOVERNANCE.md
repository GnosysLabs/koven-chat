...
- **A private space can lock encryption on for every child room.** At space creation, the founder can toggle "End-to-end encryption" — only available when the space is private — and the resulting space writes a `chat.koven.space.config` state event with `e2ee_required: true`. Every room subsequently created inside that space is forced encrypted + private regardless of what the creator picks per-room; the encryption toggle in the room create dialog is locked on. This flag is one-way: Matrix can't disable encryption on a room once enabled, so loosening the space's policy retroactively would leave a permanently encrypted backlog. Spaces created without this toggle keep the existing per-room opt-in model.
